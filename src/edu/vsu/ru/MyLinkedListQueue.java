...
public class MyLinkedListQueue<T> implements SimpleQueue<T> {
    ...
    public MyLinkedListQueue<T> duplicateElements() {
        MyLinkedListQueue<T> duplicateQueue = new MyLinkedListQueue<>();

        for (SimpleLinkedListNode current = head; current != null; current = current.next) {
            duplicateQueue.add(current.value);
            duplicateQueue.add(current.value);
        }
